...
def _become(request, mentor, mentee):
    ...
    if request.POST.get("action") == "post":
        ...
        mail_subject = " ".join([MENTOR_MAIL_SUBJECT, ": новая заявка"])
        mail_message = (
            "Получена новая заявка на менторство:\n"
        ...
        if not created:
            obj.delete()
            mail_subject = " ".join([MENTOR_MAIL_SUBJECT, ": отмена заявки"])
            mail_message = (
                "Оменена заявка на менторство:\n"
                f"Ментор: {mentor} - {mentor.member.email}\n"
                f"Менти: {mentee} - {mentee.member.email}\n"
            )
...
class MentorMenteeBaseView(LoginRequiredMixin, ListView):
    def dispatch(self, request, *args, **kwargs):
        mentor_approved = (
            request.user.mentor.approved
            if hasattr(request.user, "mentor")
            else None
        )
        mentee_approved = (
            request.user.mentee.approved
...
def sign_up(request):
    ...
    if request.POST.get("action") == "post":
        if who == "mentor":
            mail_subject = " ".join([MENTOR_MAIL_SUBJECT, ": новый ментор"])
            mail_message = (
                "Новый участник хочет стать ментором:\n"
                f"Ментор: {member.email}"
            )
        elif who == "mentee":
            mail_subject = " ".join([MENTOR_MAIL_SUBJECT, ": новый менти"])
            mail_message = (
                "Новый участник хочет стать менти:\n" f"Ментор: {member.email}"
            )
...
class SignUpBaseView(LoginRequiredMixin, FormView):
    ...
    def _send_email(self, form):
        ...
        mail_message = (
            "Новый участник хочет присоединиться:\n"
